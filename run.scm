...
(define (run-simulation small-numbers large-numbers target population-capacity generations)
    ...
    (define trees )
    (let simulation-loop ((trees (create-mutations first-tree population-capacity))
                          (target target)
...
; load the args
(define opts
    (list (args:make-option (s small)         (required: "SMALL")          "choose from 1:10 inclusive.   EG: -s 1,2,5,7")
